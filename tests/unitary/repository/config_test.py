...
def test_calls_read_env_variables_when_constructing_configuration_file(
	__mock_read_config,
	__mock_read_env_variables,
	__mock_read_config_date
):
	ConfigurationFileHandler()
	__mock_read_env_variables.assert_called_once_with()
...
@pytest.mark.parametrize(
	...
	[
		...
		pytest.param(
			[f"{variables.ENVIRONMENT_VARIABLES_PREFIX}{variables.PYTHON_HOME}", "/path/to/test"],
			id="Path variable"
		)
	],
	indirect=["__mock_environment"]
)
def test_returns_context_values_when_reading_environment_variables(
	__mock_read_config,
	__mock_read_config_date,
	__mock_get_context_values_from_file_values,
	__mock_environment
):
	variable_name = __mock_environment[0].replace(variables.ENVIRONMENT_VARIABLES_PREFIX, "")
	expected_value = __mock_get_context_values_from_file_values(__mock_environment[1])
	config_handler = ConfigurationFileHandler()
	assert (
		config_handler.values[variable_name] == expected_value
	), get_assertion_message("value type", expected_value, config_handler.values[variable_name])
...
def test_calls_now_strftime_when_writing_config(
	...
	config_handler.values = __CONFIG_VALUES.copy()
	config_handler.write_config()
	__mock_datetime_now.now.assert_called_once_with()
...
@pytest.fixture
def __mock_read_env_variables():
	with patch(
		"xmipp3_installer.repository.config.ConfigurationFileHandler.read_env_variables"
	) as mock_method:
		yield mock_method

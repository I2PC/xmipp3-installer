...
def test_calls_logger_three_times_if_first_command_succeeds_when_running_cmake_mode(
  substitute,
  __mock_run_shell_command_in_streaming,
  __mock_get_section_message,
  ...
  __mock_run_shell_command_in_streaming.return_value = None
  __mock_run_shell_command_in_streaming.side_effect = [0, 1]
  executor = ModeCompileAndInstallExecutor(__CONTEXT.copy())
...
def test_calls_logger_four_times_if_both_commands_succeed_when_running_cmake_mode(
  substitute,
  __mock_get_section_message,
  __mock_get_done_message,
  __mock_logger
...
@pytest.mark.parametrize(
  "__mock_run_shell_command_in_streaming,expected_section_code",
  [
    pytest.param(1, errors.CMAKE_COMPILE_ERROR),
    pytest.param([0, 1], errors.CMAKE_INSTALL_ERROR)
  ],
  indirect=["__mock_run_shell_command_in_streaming"]
)
def test_calls_get_error_code_if_any_command_fails_when_running_cmake_mode(
  __mock_run_shell_command_in_streaming,
  expected_section_code,
  __mock_get_error_code
):
  ModeCompileAndInstallExecutor(__CONTEXT.copy())._run_cmake_mode(__CMAKE)
  __mock_get_error_code.assert_called_once_with(1, expected_section_code)

def test_does_not_call_get_error_code_if_commands_succeed_when_running_cmake_mode(
  __mock_get_error_code
):
  ModeCompileAndInstallExecutor(__CONTEXT.copy())._run_cmake_mode(__CMAKE)
...
@pytest.mark.parametrize(
  ...
  [
    pytest.param([1, 1], (errors.CMAKE_COMPILE_ERROR, ""), id="Both commands fail"),
    pytest.param([-1, 1], (errors.INTERRUPTED_ERROR, ""), id="User interruption on compilation"),
    pytest.param([1, 0], (errors.CMAKE_COMPILE_ERROR, ""), id="Compile error"),
    pytest.param([0, -1], (errors.INTERRUPTED_ERROR, ""), id="User interruption on installation"),
